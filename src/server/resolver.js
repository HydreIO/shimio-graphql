...
export default class Resolver {
  ...
  async process_subscription({ id, operation_type, operation_name, graphql_options }) {
    this.#logger('processing subscription %O', { id, operation_type, operation_name, 'graphql_options.variableValue': graphql_options.variableValues })
    ...
    if (maybe_iterator[Symbol.asyncIterator]) {
      ...
      return { id, operation_type, operation_name, data: graphql_options.variableValues }
    }
    throw new Processing_error({ id, operation_type, operation_name }, maybe_iterator)
  }

  async process_query({ id, operation_type, operation_name, graphql_options }) {
    this.#logger('processing query %O', { id, operation_type, operation_name, 'graphql_options.variableValue': graphql_options.variableValues })
   ...
   */
  async *process_operation({ id, operations, variables }) {
    const log_op = this.#logger.extend(`op<${id}>`)
    for (const operation of operations) {
      const document = parse(operation, { noLocation: true })
   ...
   */
  async*[Symbol.asyncIterator](external_source) {
    const self = this
    yield* pipeline(
