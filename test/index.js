...
pipeline(doubt.stream(), tap_spec(), process.stdout, error => {
...
'The server is a piece of art'.doubt(async () => {
  ...
  await 'a query can use variables'.because(async () => {
    ...
    } = await client.query('query($user: User_input!) { hello(user: $user) }', {
      user: {
        name: context.name,
      },
    }).then(json)
  ...
  await 'a query is rejected when required arguments are not provided'.because(async () => {
    const {
      message,
    } = await client.query('mutation { sendMessage }').then(json)
    return message
  }).isEqualTo(PROCESSING_ERROR)
  ...
  await 'a query is rejected when arguments names are wrong'.because(async () => {
    const {
      message,
    } = await client.query('mutation($message: String) { sendMessage(mesae: $message) }').then(json)
    return message
  }).isEqualTo(PROCESSING_ERROR)
  ...
  await 'a query is rejected when arguments are of the wrong types'.because(async () => {
    const {
      message,
    } = await client.query('mutation($message: String) { sendMessage(message: $message) }', {
      message: 0,
    }).then(json)
    return message
  }).isEqualTo(PROCESSING_ERROR)
  ...
  await 'a query can include multi blocs and will pipe results to the next one'.because(async () => {
    ...
    } = await client.query(query).then(json)
    return hello
  }).isEqualTo(`Hello ${ context.name }`)
...
'The client is not vegan'.doubt(async () => {
  ...
  await 'a invalid graphql document throw a syntax error'.because(async () => {
